...
The Basics: Making a simple GET request to a URL.
Customizing: Using Query Parameters to filter or modify your request.
Sending Data: Using the POST method and a Body to send information to the server.
Identification: Using Headers and simple Authentication to prove who you are.
Handling Delays: Understanding how Timeouts prevent your workflow from getting stuck.
...
### Triggers
- Start Tutorial (`n8n-nodes-base.manualTrigger`)
- 1. The Kitchen (GET /menu) (`n8n-nodes-base.webhook`)
- 2. The Kitchen (GET /order) (`n8n-nodes-base.webhook`)
- 3. The Kitchen (POST /review) (`n8n-nodes-base.webhook`)
- 4. The Kitchen (GET /secret-dish) (`n8n-nodes-base.webhook`)
- 5. The Kitchen (GET /slow-service) (`n8n-nodes-base.webhook`)
- Respond with Secret (`n8n-nodes-base.respondToWebhook`)
- Respond: Unauthorized (401) (`n8n-nodes-base.respondToWebhook`)

## All Nodes

| Node Name | Type | Category |
|-----------|------|----------|
| Start Tutorial | `n8n-nodes-base.manualTrigger` | Trigger |
| 1. The Kitchen (GET /menu) | `n8n-nodes-base.webhook` | Trigger |
| Prepare Menu Data | `n8n-nodes-base.set` | Transform |
| 1. The Customer (GET Menu Item) | `n8n-nodes-base.httpRequest` | Output |
| 2. The Customer (GET with Query Params) | `n8n-nodes-base.httpRequest` | Output |
| 2. The Kitchen (GET /order) | `n8n-nodes-base.webhook` | Trigger |
| Prepare Cheese Pizza | `n8n-nodes-base.set` | Transform |
| Prepare Plain Pizza | `n8n-nodes-base.set` | Transform |
| 3. The Customer (POST with Body) | `n8n-nodes-base.httpRequest` | Output |
| 3. The Kitchen (POST /review) | `n8n-nodes-base.webhook` | Trigger |
| Process Review Data | `n8n-nodes-base.set` | Transform |
| 4. The Customer (GET with Headers/Auth) | `n8n-nodes-base.httpRequest` | Output |
| 4. The Kitchen (GET /secret-dish) | `n8n-nodes-base.webhook` | Trigger |
| 5. The Customer (Request with Timeout) | `n8n-nodes-base.httpRequest` | Output |
| 5. The Kitchen (GET /slow-service) | `n8n-nodes-base.webhook` | Trigger |
| Prepare Slow Response | `n8n-nodes-base.set` | Transform |
| Wait 3 seconds | `n8n-nodes-base.wait` | Other |
| IF Authorized | `n8n-nodes-base.if` | Transform |
| IF extra cheese | `n8n-nodes-base.if` | Transform |
| Base URL | `n8n-nodes-base.set` | Transform |
| Respond with Secret | `n8n-nodes-base.respondToWebhook` | Trigger |
| Respond: Unauthorized (401) | `n8n-nodes-base.respondToWebhook` | Trigger |
| ⚙️CONFIGURATION⚙️ | `n8n-nodes-base.set` | Transform |

## Connections

```
Base URL --[main]--> 1. The Customer (GET Menu Item)
Base URL --[main]--> 2. The Customer (GET with Query Params)
Base URL --[main]--> 3. The Customer (POST with Body)
Base URL --[main]--> 4. The Customer (GET with Headers/Auth)
Base URL --[main]--> 5. The Customer (Request with Timeout)
IF Authorized --[main]--> Respond with Secret
IF Authorized --[main]--> Respond: Unauthorized (401)
Start Tutorial --[main]--> ⚙️CONFIGURATION⚙️
Wait 3 seconds --[main]--> Prepare Slow Response
IF extra cheese --[main]--> Prepare Cheese Pizza
IF extra cheese --[main]--> Prepare Plain Pizza
⚙️CONFIGURATION⚙️ --[main]--> Base URL
1. The Kitchen (GET /menu) --[main]--> Prepare Menu Data
2. The Kitchen (GET /order) --[main]--> IF extra cheese
3. The Kitchen (POST /review) --[main]--> Process Review Data
4. The Kitchen (GET /secret-dish) --[main]--> IF Authorized
5. The Kitchen (GET /slow-service) --[main]--> Wait 3 seconds
```

## Workflow Summary

This workflow is triggered by Start Tutorial, 1. The Kitchen (GET /menu), 2. The Kitchen (GET /order), 3. The Kitchen (POST /review), 4. The Kitchen (GET /secret-dish), 5. The Kitchen (GET /slow-service), Respond with Secret, Respond: Unauthorized (401), goes through 9 transformation step(s), finally outputs to 1. The Customer (GET Menu Item), 2. The Customer (GET with Query Params), 3. The Customer (POST with Body), 4. The Customer (GET with Headers/Auth), 5. The Customer (Request with Timeout).
## Author Information
- **Name**: Lucas Peyrin
